...
then
	...
	if [ ${SUBMODULENAME} = "dream" -o ${SUBMODULENAME} = "vuplus" -o ${SUBMODULENAME} = "sh4" -o ${SUBMODULENAME} = "uclan" -o ${SUBMODULENAME} = "zgemma" -o ${SUBMODULENAME} = "raspberrypi" ];then
...
then
	...
	cd ..
	cd meta-raspberrypi
	echo "Checking out meta-raspberrypi upcoming branch:"
	git checkout upcoming
	git pull
	echo -e "\n"
	cd ..
